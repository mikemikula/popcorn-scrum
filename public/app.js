...
var cardAppView = require('./views/cards');
var router = require('./util/routes');

// Create main Vue app
//let app = new Vue(appView);
var cardApp = new Vue(cardAppView);

// Configure router
//router(app);
router(cardApp);

},{"./util/routes":32,"./views/cards":33,"vue/dist/vue.js":29}],31:[function(require,module,exports){
'use strict';
...
module.exports = {
    ...
    update: function update(card, callback) {
    ...
    shuffle: function shuffle(callback) {
        axios.post('/api/shuffle').then(function (response) {
            // JSON responses are automatically parsed.
            callback(null, response.data);
        }).catch(function (e) {
            callback(e, null);
    ...
    manageTimer: function manageTimer(isTiming, callback) {
...
},{"director":26}],33:[function(require,module,exports){
'use strict';

var Vue = require('vue/dist/vue.js');
var cardsDb = require('../util/cardsDb');
var socket = io();

var filters = {
    all: function all(cards) {
        return cards;
    },
    active: function active(cards) {
        return cards.filter(function (card) {
            return !card.completed;
        });
    },
    completed: function completed(cards) {
        return cards.filter(function (card) {
            return card.completed;
        });
    }
...
var app = {
    ...
    created: function created() {
        ...
        cardsDb.fetch(function (err, cards) {});
    },
    ...
    computed: {
        filteredCards: function filteredCards() {
            ...
            var totalCardsComplete = filters.completed(this.cards).length;
    ...
    methods: {
        ...
        completeCard: function completeCard(card) {
            card.completed = true;
            cardsDb.update(card, function (err, cards) {
                // this.cards = cards;
            });
        },
        ...
        updateTime: function updateTime() {
            var d = new Date();
            var changedDate = d - this.currentDate;
            this.time = Math.floor(changedDate / 1000).toString();
        },

        clickStartTimer: function clickStartTimer() {
            cardsDb.manageTimer(true, function (err, cards) {
                // this.cards = cards;
            });
        },

        clickStopTimer: function clickStopTimer() {
            cardsDb.manageTimer(false, function (err, cards) {
                // this.cards = cards;
            });
        },
        ...
        shuffleCards: function shuffleCards() {
            cardsDb.shuffle(function (err, cards) {
